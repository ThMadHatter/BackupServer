...
from backup_server.logging import setup_logging
from backup_server.config import load_all_configs, InfraSettings, SecretSettings, BackupConfig, ValidationProfile, RetentionPolicy
from backup_server.loader import load_spec
from backup_server.engine import ExecutionEngine
from backup_server.metrics import MetricsExporter
from backup_server.catalog.manager import CatalogManager, BackupCatalog
from backup_server.storage import RCloneStorage
from backup_server.exceptions import BackupEngineError, ConfigError
...
class CLIContext:
    ...
    @property
    def storage(self) -> RCloneStorage:
        if self._storage is None:
            self._storage = RCloneStorage(self.infra)
        ...
        with Lock(lock_path):
            ...
            if not dry_run:
                ...
                if infra.safe_mode:
                    ...
                    ret = cli_ctx.config.retention
                    deleted = catalog.prune_backups(
                        service_spec.name,
                        retention_days,
                        rclone_remote=infra.rclone_remote,
                        min_backups=ret.minimum_backups,
                        protect_latest=ret.protect_latest,
                        require_validation=ret.require_restore_validation
                    )
                    if deleted:
...
def list(ctx, service, auto_init):
    ...
    try:
        ...
        click.echo("-" * 90)
        for b in backups:
            status = "Validated" if b.get("validated") else "Success"
            click.echo(f"{b['id']:<5} {b['service']:<15} {b['timestamp']:<20} {b['run_id']:<40} {status}")
    except BackupEngineError as e:
        click.echo(f"Error: {str(e)}", err=True)
        sys.exit(1)
...
def doctor(ctx):
    ...
    try:
        infra = cli_ctx.infra
        click.echo("--- System Health Check (Doctor) ---")

        # Check directories
        for name, path in [("Base", infra.backup_base_dir), ("Staging", infra.staging_dir), ("Metrics", infra.metrics_dir)]:
            if path.exists():
                if os.access(path, os.W_OK):
                    click.echo(f"[*] {name} directory OK: {path}")
                else:
                    click.echo(f"[X] {name} directory NOT WRITABLE: {path}")
            else:
                click.echo(f"[X] {name} directory MISSING: {path}")

        # Check catalog
        if infra.catalog_path.exists():
             click.echo(f"[*] Catalog exists: {infra.catalog_path}")
             # Check sqlite integrity
             import sqlite3
             with sqlite3.connect(infra.catalog_path) as conn:
                 res = conn.execute("PRAGMA integrity_check").fetchone()[0]
                 if res == "ok":
                     click.echo("[*] Catalog integrity: OK")
                 else:
                     click.echo(f"[X] Catalog integrity FAILED: {res}")
        else:
             click.echo(f"[X] Catalog MISSING: {infra.catalog_path}")

        # Check storage
        try:
            cli_ctx.storage.list("")
            click.echo("[*] RClone storage: REACHABLE")
        except Exception as e:
            click.echo(f"[X] RClone storage UNREACHABLE: {str(e)}")

        # Check disk space
        usage = shutil.disk_usage(infra.backup_base_dir)
        click.echo(f"[*] Disk space: {usage.free / (1024**3):.2f} GB free of {usage.total / (1024**3):.2f} GB")

    except Exception as e:
        click.echo(f"Doctor failed: {str(e)}", err=True)

@cli.command()
@click.pass_context
def audit(ctx):
    """Audit the catalog and detect orphaned artifacts."""
    cli_ctx: CLIContext = ctx.obj
    try:
        catalog = cli_ctx.catalog
        backups = catalog.get_backups()
        click.echo(f"--- Catalog Audit ({len(backups)} entries) ---")

        # This is a simple audit that just lists what's in the catalog
        # In a real system it would compare with remote storage
        for b in backups:
            click.echo(f"Run {b['run_id']} ({b['service']}): {b['artifact_path']} [Validated: {bool(b['validated'])}]")

        # Detect orphans would require full remote listing

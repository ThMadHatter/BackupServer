...
from backup_server.loader import ServiceSpec, StepSpec, HookSpec
from backup_server.context import ExecutionContext
from backup_server.dag import ExecutionDAG
from backup_server.primitives.base import Primitive
...
class ExecutionEngine:
    def __init__(self, infra=None, config=None, catalog=None, storage=None, dry_run: bool = False, staging_base: Optional[Path] = None):
        self.infra = infra
        self.config = config
        self.catalog = catalog
        self.storage = storage
        self.dry_run = dry_run
        self.staging_base = staging_base or (infra.staging_dir if infra else Path("/tmp/backup-engine/staging"))
    ...
    def run_service(self, spec: ServiceSpec, operation: str = "backup", initial_context: Dict[str, Any] = {}, skip_steps: List[str] = []):
        ...
        try:
            ...
            if operation == "backup":
                ...
                self.generate_manifest(spec, log)
            elif operation == "restore":
                self.execute_steps(spec.hooks.post_restore, log, "post_restore")
                self._validate_restore_integrity(log)
    ...
    def _validate_restore_integrity(self, log: Any):
        if self.dry_run:
            return

        # Post-restore integrity check: if we have expected_checksum, we verify the restored file
        # This assumes the restore operation resulted in a single file that can be checked.
        # In a real system, we'd check the entire target directory.
        dest_path = self.context.variables.get("restore_dest_path")
        expected_checksum = self.context.variables.get("expected_checksum")

        if dest_path and expected_checksum:
            log.info("Performing post-restore integrity validation", path=dest_path)
            sha256 = hashlib.sha256()
            ...
            actual_checksum = sha256.hexdigest()
            if actual_checksum != expected_checksum:
                 raise RestoreSafetyError(f"Restore integrity failed! Recalculated checksum {actual_checksum} does not match original {expected_checksum}")
            log.info("Post-restore integrity validation passed")
    ...
    def generate_manifest(self, spec: ServiceSpec, log: Any):
        if not self.context:
            return
        ...
        manifest = {
            ...
            "metadata": self.context.execution_metadata
        }

        manifest_path = self.context.staging_dir / "manifest.json"
        if not self.dry_run:
            with open(manifest_path, "w") as f:
                json.dump(manifest, f, indent=2)

        self.context.execution_metadata["manifest"] = manifest
        log.info("Run manifest generated", path=str(manifest_path))

    def execute_step(self, step: StepSpec, log: Any):
        log = log.bind(step=step.name, type=step.type)
        step_entry = {"name": step.name, "type": step.type, "start_time": datetime.now().isoformat()}

        if self.dry_run:
            log.info("Dry run: Skipping step execution")
            step_entry["status"] = "dry_run"
        ...
        while attempt <= retries:
            try:
                ...
                if step.store_result:
                    self.context.variables[step.store_result] = result
